...
def check_2(lst_obj):
    ...
    Сложность: O(N*N*1+1)=O(N^2).
    """
    for j in range(len(lst_obj)):          # O(N+len(lst_obj))=O(N)
        if lst_obj[j] in lst_obj[j+1:]:    # O(1+1+N+N)=O(N)
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    Сложность: O(len(lst_obj)+NlogN+N*N*1+1)=O(N^2)
    """
